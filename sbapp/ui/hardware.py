...
class Hardware():
        ...
        if not self.app.hardware_ready:
            ...
            if RNS.vendor.platformutils.get_platform() == "android":
                if not self.app.sideband.getpersistent("service.is_controlling_connectivity"):
                    ...
                    info =  "When using external hardware for communicating, you may configure various parameters, such as channel settings, modulation schemes, interface speeds and access parameters. You can set up these parameters per device type, and Liberty Chat Pro will apply the configuration when opening a device of that type.\n\n"
                    info += "Hardware configurations can also be exported or imported as [i]config motes[/i], which are self-contained plaintext strings that are easy to share with others. When importing a config mote, Liberty Chat Pro will automatically set all relevant parameters as specified within it.\n\n"
                    info += "For changes to hardware parameters to take effect, you must shut down and restart Liberty Chat Pro.\n"
                    self.hardware_screen.ids.hardware_info.text = info
    ...
    def rnode_preset_action(self, sender=None):
        from kivymd.uix.menu import MDDropdownMenu
        from lcs.lora_presets import PRESETS, preset_summary
        ...
        for p in PRESETS:
            items.append({
                ...
                "text": p["name"],
                "secondary_text": preset_summary(p),
                "height": dp(58),
                "on_release": (lambda k=p["key"]: self.apply_rnode_preset(k)),
            })

        self.preset_menu = MDDropdownMenu(
            caller=self.hardware_rnode_screen.ids.hardware_rnode_preset_button,
            items=items, width_mult=5, max_height=dp(420),
        )
        self.preset_menu.open()

    def apply_rnode_preset(self, key):
        from lcs.lora_presets import get_preset, preset_summary
        try: self.preset_menu.dismiss()
    ...
    def update_rnode_preset_display(self):
        ...
        try:
            cfg = self.app.sideband.config
            key = match_preset(cfg["hw_rnode_bandwidth"],
                               cfg["hw_rnode_spreading_factor"],
                               cfg["hw_rnode_coding_rate"])
            preset = get_preset(key)
            ids = self.hardware_rnode_screen.ids
            ids.hardware_rnode_preset_button.text = "Modem preset: "+preset["name"]
            ids.hardware_rnode_preset_summary.text = preset_summary(preset)
        except Exception as e:
            RNS.log("Could not update preset display: "+str(e), RNS.LOG_DEBUG)
    ...
    def hardware_rnode_save(self):
        ...
        try:
            _txp = int(self.hardware_rnode_screen.ids.hardware_rnode_txpower.text)
            from lcs.lora_presets import TX_POWER_MIN_DBM, TX_POWER_MAX_DBM
            _txp = max(TX_POWER_MIN_DBM, min(TX_POWER_MAX_DBM, _txp))
            self.app.sideband.config["hw_rnode_tx_power"] = _txp
            self.hardware_rnode_screen.ids.hardware_rnode_txpower.text = str(_txp)
        except: pass
        ...
        ScrollView:
            ...
            MDBoxLayout:
                ...
                MDBoxLayout:
                    ...
                    size_hint_y: None
                    height: self.minimum_height
                    padding: [dp(0), dp(0), dp(0), dp(18)]

                    MDRectangleFlatIconButton:
                        id: hardware_rnode_preset_button
                        icon: "tune-variant"
                        text: "Modem preset"
                        padding: [dp(0), dp(14), dp(0), dp(14)]
                        icon_size: dp(24)
                        font_size: dp(16)
                        size_hint: [1.0, None]
                        on_release: root.delegate.rnode_preset_action(self)

                    MDLabel:
                        id: hardware_rnode_preset_summary
                        text: ""
                        font_size: dp(13)
                        size_hint_y: None
                        height: dp(22)
