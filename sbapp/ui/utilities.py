...
class Utilities():
    def __init__(self, app):
        ...
        info += "that can be helpful while using Liberty Chat Pro and Reticulum."
    ...
    def buy_rnode_action(self, sender=None):
        import webbrowser
        from lcs import LCS_STORE_URL
        try:
            webbrowser.open(LCS_STORE_URL)
        except Exception as e:
            RNS.log("Could not open store URL: "+str(e), RNS.LOG_ERROR)


    ### RNode Config Tool
    ######################################

    def config_tool_action(self, sender=None):
        ...
        dialog = MDDialog(
            title="RNode Config Tool",
            text="Launch the LCS RNode Config Tool to flash and configure your RNode. This starts the built-in repository server and opens the tool in your browser.\n\nThe tool requires WebSerial, so use Chrome or Edge. Firefox and iOS Safari cannot talk to serial devices.",
            buttons=[ no_button, yes_button ],
            # elevation=0,
        )
        def dl_yes(s):
            dialog.dismiss()
            self.app.wants_config_tool_launch = True
        ...
        ScrollView:
            ...
            MDBoxLayout:
                ...
                MDBoxLayout:
                    ...
                    MDRectangleFlatIconButton:
                        id: config_tool_button
                        icon: "radio-handheld"
                        text: "RNode Config Tool"
                        padding: [dp(0), dp(14), dp(0), dp(14)]
                        icon_size: dp(24)
                        font_size: dp(16)
                        size_hint: [1.0, None]
                        on_release: root.delegate.config_tool_action(self)
                        disabled: False
                    ...
                    MDRectangleFlatIconButton:
                        ...
                        font_size: dp(16)
                        size_hint: [1.0, None]
                        on_release: root.delegate.buy_rnode_action(self)
                        disabled: False
