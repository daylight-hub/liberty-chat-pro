...
class SidebandCore():
    ...
    def __init__(self, owner_app, config_path = None, is_service=False, is_client=False, android_app_dir=None, verbose=False, quiet=False, owner_service=None, service_context=None, is_daemon=False, load_config_only=False, rns_config_path=None):
        ...
        if config_path == None:
            ...
            # self.app_dir     = os.path.join(h_dir, ".config", "libertychatpro")
            # if self.app_dir.startswith("file://"): self.app_dir = self.app_dir.replace("file://", "")
            self.app_dir     = os.path.join(plyer.storagepath.get_home_dir(), ".config", "libertychatpro")
            if self.app_dir.startswith("file://"): self.app_dir = self.app_dir.replace("file://", "")
        else:
        ...
        if RNS.vendor.platformutils.is_android():
            self.app_dir = os.path.join(android_app_dir, "network.lcs.libertychatpro", "files")
            self.cache_dir = os.path.join(self.app_dir, "cache")
        ...
        try:
            if not os.path.isfile(self.config_path):
                ...
                try:
                    ...
                    RNS.log(f"An error occurred while loading existing Liberty Chat Pro configuration: {e}", RNS.LOG_ERROR)
            ...
            RNS.log(f"Error while configuring Liberty Chat Pro: {e}", RNS.LOG_ERROR)
            RNS.trace_exception(e)
            ...
            if self.config["config_template"] != None:
                try:
                    if not os.path.isfile(os.path.join(self.rns_configdir, "config_template_invalid")):
                        ...
                        self.config_template = rns_config
                        if self.is_service:
                            self.setstate("hardware_operation.error", "At the previous start, Liberty Chat Pro could not initialise Reticulum. Custom configuration template loading has been temporarily disabled. Please check and fix any errors in your configuration template.")
            ...
            if RNS.vendor.platformutils.is_linux():
                try:
                    if not self.is_daemon:
                        ...
                        de_filename = "network.lcs.libertychatpro.desktop"
                        ...
                        icn_target = os.path.join(icon_dir, "network.lcs.libertychatpro.png")
    ...
    def __init_config(self):
        RNS.log("Creating new Liberty Chat Pro configuration...")
        ...
        self.config["connect_tcp_host"] = "public.lcs.network"
        self.config["connect_tcp_port"] = "4244"
        ...
        # Hardware
        self.config["hw_rnode_frequency"] = 914875000
        self.config["hw_rnode_modulation"] = "LoRa"
        self.config["hw_rnode_bandwidth"] = 250000
        self.config["hw_rnode_spreading_factor"] = 11
        self.config["hw_rnode_coding_rate"] = 5
        self.config["hw_rnode_tx_power"] = 22
        self.config["hw_rnode_preset"] = "long_fast"
    ...
    def __load_config(self):
        RNS.log("Loading Liberty Chat Pro identity...", RNS.LOG_DEBUG)
        self.identity = RNS.Identity.from_file(self.identity_path)

        self.rpc_addr = f"\0sideband/rpc"
        self.rpc_key  = RNS.Identity.full_hash(self.identity.get_private_key())

        RNS.log("Loading Liberty Chat Pro configuration... "+str(self.config_path), RNS.LOG_DEBUG)
        ...
        if not "hw_rnode_frequency" in self.config:                    self.config["hw_rnode_frequency"] = 914875000
        if not "hw_rnode_modulation" in self.config:                   self.config["hw_rnode_modulation"] = "LoRa"
        if not "hw_rnode_bandwidth" in self.config:                    self.config["hw_rnode_bandwidth"] = 250000
        if not "hw_rnode_spreading_factor" in self.config:             self.config["hw_rnode_spreading_factor"] = 11
        if not "hw_rnode_coding_rate" in self.config:                  self.config["hw_rnode_coding_rate"] = 5
        if not "hw_rnode_tx_power" in self.config:                     self.config["hw_rnode_tx_power"] = 22
        if not "hw_rnode_preset" in self.config:                       self.config["hw_rnode_preset"] = "long_fast"
        ...
        if not "voice_enabled" in self.config:                         self.config["voice_enabled"] = True
    ...
    def __reload_config(self):
        RNS.log("Reloading Liberty Chat Pro configuration... ", RNS.LOG_DEBUG)
    ...
    def __save_config(self, no_thread=False):
        RNS.log("Saving Liberty Chat Pro configuration...", RNS.LOG_DEBUG)
        def save_function():
            ...
            try:
                ...
                self.saving_configuration = False
                RNS.log("Error while saving Liberty Chat Pro configuration: "+str(e), RNS.LOG_ERROR)
        ...
        RNS.log("Liberty Chat Pro configuration saved", RNS.LOG_DEBUG)

    def __load_plugins(self):
        try:
            from lcs.bootstrap import install_bundled_plugins
            install_bundled_plugins(self)
        except Exception as e:
            RNS.log("LCS plugin bootstrap failed: "+str(e), RNS.LOG_ERROR)
        ...
        if plugins_enabled:
            if plugins_path != None:
                RNS.log("Loading Liberty Chat Pro plugins...", RNS.LOG_DEBUG)
                if os.path.isdir(plugins_path):
                    for file in os.listdir(plugins_path):
                        if file.lower().endswith(".py"):
                            plugin_globals = {}
                            plugin_globals["SidebandServicePlugin"] = SidebandServicePlugin
                            plugin_globals["Liberty Chat ProCommandPlugin"] = SidebandCommandPlugin
                            plugin_globals["Liberty Chat ProTelemetryPlugin"] = SidebandTelemetryPlugin
                ...
                if notifications_permitted:
                    try:
                        if RNS.vendor.platformutils.get_platform() == "android":
                            ...
                            if not RNS.vendor.platformutils.is_windows(): plyer.notification.notify(title, content, app_name="Liberty Chat Pro", app_icon=self.icon_48)
                            else: plyer.notification.notify(title, content, app_name="Liberty Chat Pro", app_icon=self.icon_windows)
                    except Exception as e:
                        ...
                        if hasattr(self.owner_app, "usb_devices") and self.owner_app.usb_devices != None:
                            if len(self.owner_app.usb_devices) > 0:
                                try:
                                    ...
                                    self.setstate("hardware_operation.error", "An error occurred while trying to communicate with the device. Please make sure that Liberty Chat Pro has been granted permissions to access the device.\n\nThe reported error was:\n\n[i]"+str(e)+"[/i]")
                            else:
                                RNS.log("Could not execute RNode Bluetooth control command, no USB devices available", RNS.LOG_ERROR)
    ...
    def start(self):
        ...
        RNS.log("Liberty Chat Pro Core "+str(self)+" "+str(self.version_str)+"started")
...
# running configuration for Liberty Chat Pro's
# internal RNS instance. Incorrect changes
# or addition here may cause Liberty Chat Pro to
# fail starting up or working properly.
#
# If Liberty Chat Pro detects that Reticulum
...
# not relevant in Liberty Chat Pro.
[logging]
  loglevel = 3
