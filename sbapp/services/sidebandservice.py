...
class SidebandService():
    ...
    def android_notification(self, title="", content="", ticker="", group=None, context_id=None):
        try:
            if android_api_version < 26: return
                ...
                package_name = "network.lcs.libertychatpro"
                ...
                if not title or title == "": channel_name = "Liberty Chat Pro"
    ...
    def get_connectivity_status(self):
        if self.sideband.reticulum.is_connected_to_shared_instance:
            return "[size=22dp][b]Connectivity Status[/b][/size]\n\nLiberty Chat Pro is connected via a shared Reticulum instance running on this system. Use the rnstatus utility to obtain full connectivity info."
        else:
            ws = "Disabled"
            ts = "Disabled"
            i2s = "Disabled"
