...
import sys
import argparse
parser = argparse.ArgumentParser(description="Liberty Chat Pro LXMF Client")
...
def apply_ui_scale():
    ...
    try:
        if RNS.vendor.platformutils.is_android():
            import plyer
            ui_scale_path = os.path.join(plyer.storagepath.get_application_dir(), "network.lcs.libertychatpro", "files", "app_storage", "ui_scale")
        else:
            if config_path == None:
                import sbapp.plyer as plyer
                ui_scale_path = os.path.join(plyer.storagepath.get_home_dir(), ".config", "libertychatpro", "app_storage", "ui_scale")
...
class SidebandApp(MDApp):
    ...
    PKGNAME  = "network.lcs.libertychatpro"
    ...
    def __init__(self, **kwargs):
        ...
        self.rnode_config_tool_url = None
    ...
    def start_service(self):
        ...
        if RNS.vendor.platformutils.get_platform() == "android":
            self.android_service = autoclass('network.lcs.libertychatpro.ServiceSidebandservice')
        ...
        if RNS.vendor.platformutils.is_android():
            if self.sideband.getstate("android.power_restricted", allow_cache=False):
                ...
                def pm_job(dt):
                    ...
                    requestIntent.setData(Uri.parse("package:network.lcs.libertychatpro"))
    ...
    def share_image(self, image, filename):
        if RNS.vendor.platformutils.get_platform() == "android":
            ...
            try:
                ...
                image_uri = FileProvider.getUriForFile(mActivity, "network.lcs.libertychatpro.provider", i_file)
        ...
        if android_api_version < 30:
            ...
            if Environment.isExternalStorageManager():
                ...
                dialog = MDDialog(
                    ...
                    text="Liberty Chat Pro needs external storage permission to read offline map files.\n\nOn this Android version, the Manage All Files permission is needed, since normal external storage permission is no longer supported.\n\nLiberty Chat Pro will only ever read and write to files you select, and does not read any other data from your system.",
                    buttons=[ ok_button ],
                )
                def dl_ok(s):
                    dialog.dismiss()
    ...
    def start_bluetooth_scan(self):
        if not self.has_location_permissions():
            if not hasattr(self, "permission_dialog") or self.permission_dialog == None:
                permission_dialog_text = "[b]Missing Permissions[/b]\n\nOn this version of Android, location permission is required to scan for Bluetooth devices. Yes, this is silly, but there's no way around it.\n\nIf you don't want Liberty Chat Pro to have location access, you can disable this permission after scanning and pairing your RNode, and everything will still work, as it is only the scanning process that requires this."
            ...
            if RNS.vendor.platformutils.is_android() and not self.sideband.service_available():
                if time.time() - self.service_last_available > SidebandApp.SERVICE_TIMEOUT:
                    if self.app_state == SidebandApp.ACTIVE:
                        info_text = "The Reticulum and LXMF service seem to have disappeared, and Liberty Chat Pro is no longer connected. This should not happen, and probably indicates a bug in the background service. Please restart Liberty Chat Pro to regain connectivity."
                        ok_button = MDRectangleFlatButton(text="OK",font_size=dp(18))
                        dialog = MDDialog(
                            title="Error",
                            text=info_text,
    ...
    def on_start(self):
        ...
        self.root.ids.app_version_info.text = "Liberty Chat Pro v"+__version__+variant_str
        ...
        try:
            ...
            if RNS.vendor.platformutils.get_platform() == "android":
                ...
                if self.sideband.reticulum.is_connected_to_shared_instance:
                    connectivity_status = "Liberty Chat Pro is connected via a shared Reticulum instance running on this system. Use the [b]rnstatus[/b] utility to obtain full connectivity info."
                else:
                    connectivity_status = "Liberty Chat Pro is currently running a standalone or master Reticulum instance on this system. Use the [b]rnstatus[/b] utility to obtain full connectivity info."
    ...
    def open_ingest_lxm_dialog(self, sender=None):
        try:
            ...
            dialog = MDDialog(
                ...
                text="You can read LXMF paper messages into this program by scanning a QR-code containing the message with your device camera or QR-scanner app, and then opening the resulting link in Liberty Chat Pro.\n\nAlternatively, you can copy an [b]lxm://[/b] link from any source to your clipboard, and ingest it using the [i]Read LXM[/i] button below.",
    ...
    def information_init(self):
        if not self.root.ids.screen_manager.has_screen("information_screen"):
            ...
            def link_exec(sender=None, event=None):
                targets = {"link": "https://unsigned.io/donate",
                           "lcs":  "https://www.lcs.network"}
                url = targets.get(event, "https://unsigned.io/donate")
                def lj():
                    webbrowser.open(url)
            ...
            str_comps += "\n\n[b]Liberty Chat Pro[/b] is a modified version of [b]Sideband[/b], produced by Liberty Communication Systems, Inc. ([u][ref=lcs]https://www.lcs.network[/ref][/u]). It is not produced, endorsed or supported by Mark Qvist."
            str_comps += "\n\nThe Sideband app is Copyright © 2026 Mark Qvist / unsigned.io, licensed under CC BY-NC-SA 4.0. As a derivative work, Liberty Chat Pro is released under the same licence. Go to [u][ref=link]https://unsigned.io/donate[/ref][/u] to support the upstream project."
            str_comps += "\n\nPermission is granted to freely share and distribute binary copies of "+self.root.ids.app_version_info.text+", so long as no payment or compensation is charged for said distribution or sharing.\n\nIf you were charged or paid anything for this copy, please report it to [b]license@unsigned.io[/b].\n\nTHIS IS EXPERIMENTAL SOFTWARE - IT COMES WITH ABSOLUTELY NO WARRANTY - USE AT YOUR OWN RISK AND RESPONSIBILITY"
    ...
    def settings_init(self, sender=None):
        if not self.settings_ready:
            ...
            info1_text += "\n\nYou can manually specify which [b]Propagation Node[/b] to use, but if none is specified, Liberty Chat Pro will "
        ...
        if not self.connectivity_ready:
            ...
            if RNS.vendor.platformutils.get_platform() == "android":
                if not self.sideband.getpersistent("service.is_controlling_connectivity"):
                    info =  "Liberty Chat Pro is connected via a shared Reticulum instance running on this system.\n\n"
                    ...
                    info =  "By default, Liberty Chat Pro will try to discover and connect to any available Reticulum networks via active WiFi and/or Ethernet interfaces. If any Reticulum Transport Instances are found, Liberty Chat Pro will use these to connect to wider Reticulum networks. You can disable this behaviour if you don't want it.\n\n"
                    ...
                    info += "For changes to connectivity to take effect, you must either restart the RNS service, or completely shut down and restart Liberty Chat Pro.\n"
                    self.connectivity_screen.ids.connectivity_info.text = info
                ...
                if self.sideband.reticulum.is_connected_to_shared_instance:
                    ...
                    info =  "Liberty Chat Pro is currently running a standalone or master Reticulum instance on this system.\n\n"
                    info += "To get connectivity status, use the [b]rnstatus[/b] utility.\n\n"
                    info += "To configure connectivity, edit the configuration file located at:\n\n"
                    if not RNS.vendor.platformutils.is_windows(): info += str(RNS.Reticulum.configpath)
                    else:                                         info += str(RNS.Reticulum.configpath.replace("/", "\\"))
    ...
    def repository_update_info(self, sender=None):
        info =  "Liberty Chat Pro includes a small repository of useful software and guides related to the Liberty Chat Pro and Reticulum ecosystem. You can start this repository to allow other people on your local network to download software and information directly from this device, without needing an Internet connection.\n\n"
        info += "If you want to share the Liberty Chat Pro application itself via the repository server, you must first download it into the local repository, using the \"Update Content\" button below.\n\n"
        ...
        if self.sideband.webshare_server != None:
            ...
            if ips == None or len(ips) == 0:
                ...
                for ip in ips:
                    ...
                    if self.repository_url == None:
                        ...
                        self.rnode_config_tool_url = ipurl+"mirrors/rnode-config-tool/RNode_Config_Tool.html"
            ...
            def cb(dt):
                ...
                if hasattr(self, "wants_config_tool_launch") and self.wants_config_tool_launch == True:
                    self.wants_config_tool_launch = False
                    if self.rnode_config_tool_url != None:
                        def lj():
                            webbrowser.open(self.rnode_config_tool_url)
                        threading.Thread(target=lj, daemon=True).start()
            ...
            try:
                ...
                if True:
                    ...
                    try:
                        ...
                        self.repository_screen.ids.repository_update.text = f"Downloading Liberty Chat Pro APK release info failed with the error:\n"+str(e)
                        return
    ...
    def plugins_init(self):
        if not self.root.ids.screen_manager.has_screen("plugins_screen"):
            ...
            info1 = "You can extend Liberty Chat Pro functionality with command and service plugins. This lets you to add your own custom functionality, or add community-developed features.\n"
            info2 = "[b]Take extreme caution![/b]\nIf you add a plugin that you did not write yourself, make [b]absolutely[/b] sure you know what it is doing! Loaded plugins have full access to your Liberty Chat Pro application, and should only be added if you are completely certain they are trustworthy.\n\n"
            ...
            info2 += "By using [i]Service Plugins[/i], you can start additional services or programs within the Liberty Chat Pro application context, that other plugins (or Liberty Chat Pro itself) can interact with.\n\n"
            info2 += "With [i]Telemetry Plugins[/i], you can add custom telemetry from external devices and services to the Liberty Chat Pro telemetry system.\n\n"
            info2 += "Restart Liberty Chat Pro for changes to these settings to take effect."
    ...
    def map_settings_action(self, sender=None, direction="left"):
        if not self.root.ids.screen_manager.has_screen("map_settings_screen"):
            ...
            self.map_settings_screen.ids.map_config_info.text = "\n\nLiberty Chat Pro can use map sources from the Internet, or a map source stored locally on this device in MBTiles format."
...
def run():
    if args.daemon:
        RNS.log("Starting Liberty Chat Pro in daemon mode")
        sideband = SidebandCore(
            None,
            config_path=args.config,
            is_client=False,
        ...
        if args.interactive:
            ...
            if importlib.util.find_spec('prompt_toolkit') != None:
                ...
                print("Could not start Liberty Chat Pro console, since the \"prompt-toolkit\" module is not available")
